...
function rssMaddeler(xml, sinir){
  ...
  while((m = re.exec(xml)) && maddeler.length < (sinir || 15)){
    ...
    const kay = coz((b.match(/<source[^>]*>([\s\S]*?)<\/source>/) || [])[1]);
    if(!t) continue;
  ...
  async fetch(req, env) {
    ...
    try {
      ...
      if (yol === '/haber') {
        ...
        const r = await fetch('https://news.google.com/rss/search?q=' + q + '&hl=tr&gl=TR&ceid=TR:tr',
          { headers: { 'User-Agent': 'Mozilla/5.0 (compatible; FinansMotoru/1.0)' } });
        if (!r.ok) return json({ error: 'haber kaynağı HTTP ' + r.status }, 502);
        const maddeler = rssMaddeler(await r.text(), 15);
        const veri = { kod, basliklar: maddeler, not: 'genel haber araması — başlık düzeyi' };
      ...
      return json({
        ...
        uclar: ['/ai', '/haber?symbol=', '/check', '/paket', '/seri?code=&start=', '/gruplar?kelime=', '/liste?grup=', '/yahoo?symbol=', '/tefas?fon=', '/tefaskesif', '/tefasjs?js=', '/temel?symbol=']
      });
    } catch (e) {
