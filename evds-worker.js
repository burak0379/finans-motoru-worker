...
const AI_SISTEM =
  ...
  '(6) ÇIKTIN yalnızca akıcı Türkçe DÜZ YAZI tek paragraf olsun — JSON, süslü parantez, tırnaklı alan adı (ör. "_yuzde") ASLA yazma; sayıları cümle içinde doğal biçimde ver.';
...
export default {
  async fetch(req, env) {
    ...
    try {
      ...
      /* ── /yedek: bulut eşitleme (v2.11) — KV + kişisel anahtar ── */
      if (yol === '/yedek') {
        if (!env.FM_KV)
          return json({ error: 'KV bağlı değil — kurulum: Cloudflare panel → Storage & Databases → KV → Create namespace (FM_DEPO), sonra repodaki wrangler yapılandırmasına FM_KV bağlaması (talimat uygulamada)' }, 500);
        if (!env.SYNC_KEY)
          return json({ error: 'SYNC_KEY tanımlı değil — Cloudflare Secrets\u0027a kendi belirlediğin eşitleme parolasını SYNC_KEY adıyla ekle' }, 500);
        const anahtar = req.headers.get('X-Sync-Key') || url.searchParams.get('k') || '';
        if (anahtar !== env.SYNC_KEY) return json({ error: 'eşitleme anahtarı yanlış' }, 403);
        if (req.method === 'PUT' || req.method === 'POST') {
          ...
          await env.FM_KV.put('yedek', govde);
          const meta = { t: new Date().toISOString(), boyutKB: Math.round(govde.length/1024) };
          await env.FM_KV.put('yedekMeta', JSON.stringify(meta));
          return json({ ok: true, ...meta });
        }
        if (url.searchParams.get('meta')) {
          const m = await env.FM_KV.get('yedekMeta');
          return json(m ? JSON.parse(m) : { bos: true });
        }
        const v = await env.FM_KV.get('yedek');
        if (v === null) return json({ error: 'bulutta yedek yok — önce diğer cihazdan ☁ Buluta Yedekle' }, 404);
